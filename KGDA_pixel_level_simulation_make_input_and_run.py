...
if __name__ == '__main__':
    ...
    # weatherFiles = glob.glob(r'E:/My Drive/GEE DaymetV4_Champaign/%s*.tif'%year)
    # weatherFiles.sort()
    ...
    genEn = makeInput_ensemble_parallel(paraMode='intevalAcc',year=year)
    ...
    for crop in ['soybean']:
        ...
        breakPoint = True
        if breakPoint:
            breakPoint_dic = util.load_object('pixel_simulation/res_soybean_%s_breakPoint.pkl'%year)
            unfinishedPoint = [k for k,j in breakPoint_dic['DA'].items() if len(j)==0]
            points = points.iloc[unfinishedPoint[0]:]
            FIDall = unfinishedPoint
            
        for n,p in enumerate(zip(points['x'].tolist(),points['y'])):
            if n == 0:
                tmp = [p]
                tmp2 = [FIDall[n]]
            elif n%DAbatch == 0:
        ...
        for oneBatch,FID in zip(DAbatchList,FIDbatchList):
            ...
            if eps%10==0:
                print('%s Batch %s, take %.2f s for one-batch, %s/%s pixels finished.'%(crop,eps,(finishTime0-startTime0),eps*DAbatch,len(points)))
            
                # save break point
                resDic_midPoint={}
                resDic_midPoint['DA'] = yieldTimeSeires
                resDic_midPoint['op'] = yieldTimeSeires_op
                util.save_object(resDic_midPoint,'%s/res_%s_%s_breakPoint.pkl'%(outPath,crop,year))
                
        # save results
        resDic={}
        resDic['DA'] = yieldTimeSeires
        resDic['op'] = yieldTimeSeires_op
        util.save_object(resDic,'%s/res_%s_%s.pkl'%(outPath,crop,year))
