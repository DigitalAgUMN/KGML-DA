# -*- coding: utf-8 -*-
"""
Created on Mon Mar 18 15:45:02 2024
...
v3.8: output carbon cycle, added the soil property perturbing flag
"""
...
import scipy.signal
import KGDA_Test_county_plot as plot
...
class makeInput_ensemble_parallel():  
    ...
    def disturbPara(self,inputEpisodes, ensemble_n = 100,FIPSbatch=None,disturbPD=False,CV_para=0.1, CV_PD = 0.05,obs_smooth_dic_all=None,
                        measDOY=None,year=None,obsType=None,startYear=2000,cropTypes=None,perturb_soil=True):
        # np.random.seed(0)
        daily_En_list = []
        validFIPS = []
        measValue = []
        ...
        for inputEpisode,FIPS,crop in zip(inputEpisodes,FIPSbatch,cropTypes):
            ...
            if perturb_soil:
                fixP.extend([13,14,15,16,17,18,19])
...
if __name__ == '__main__':
    ...
    globalf=True
    perturb_soil = True#False
    for paraMode in ['intevalAcc']:#'default''defaultV2''intevalAcc'#,'eachYear'#'previousYear'#'inteval'#'global'#'eachYear'#'intevalAcc' 
    
        for obsMode in ['LAI']:#'GPP_ET','GPP_LAI_ET'
            ...
            for modelv in modelVersion:
                for maskMode in modeList:
                    note = 'county_yield_parallel_%s_%s_obsInterval%s_countyMerge_%s_case%s_LAI_ass_vegetative_stage'%(modelv[0],maskMode,obsIntertval,paraMode,case)
                    projectName = '%s-%s'%(note,now)
                    outFolder = 'F:/countyYieldResult/EnsembleSigma/LAItest/%s'%(projectName)
                    ...
                    for crop in ['corn','soy']:
                        ...
                        measDOY = [i for i in range(145+4,210, obsIntertval)]
                        FIPSbatchList = []
                        ...
                        for batch,FIPSbatch in enumerate(FIPSbatchList):
                            ...
                            for year in range(yearRange[0],yearRange[1]+1):
                                ...
                                enList, validFIPS, measValue = genEn.disturbPara(inputEpisode_reset,ensemble_n = ensemble_n,FIPSbatch=FIPSbatch,
                                                                      ...
                                                                      measDOY=measDOY,year=year,obsType=obsType,cropTypes=cropTypes,
                                                                      startYear=yearRange[0],perturb_soil=perturb_soil)
                                ...
                                _, LAI_mean_list, _ = ensembleProcess_parallel(sigmas_enkf,i=-2,inputData=genEn)
                                _, ET_mean_list, _ = ensembleProcess_parallel(sigmas_enkf,i=1,inputData=genEn)
                                _, GPP_mean_list, _ = ensembleProcess_parallel(sigmas_enkf,i=2,inputData=genEn)
                                _, Reco_mean_list, _ = ensembleProcess_parallel(sigmas_enkf,i=5,inputData=genEn)
                                _, NEE_mean_list, _ = ensembleProcess_parallel(sigmas_enkf,i=6,inputData=genEn)
                                ...
                                if year == yearRange[0]:
                                    ...
                                    for s in FIPSbatch:
                                        if s in validFIPS:
                                            ...
                                            GPPTimeSeires[s] =  [GPP_mean_list[n]]
                                            RecoTimeSeires[s] =  [Reco_mean_list[n]]
                                            NEETimeSeires[s] =  [NEE_mean_list[n]]
                                            LAITimeSeires[s] =  [LAI_mean_list[n]]
                                            ETTimeSeires[s] =  [ET_mean_list[n]]
                                            n+=1
                                        else:
                                            yieldTimeSeires[s] = [None]
                                            yieldTimeSeires_std[s] = [None]
                                            GPPTimeSeires[s] = [None]
                                            RecoTimeSeires[s] =  [None]
                                            NEETimeSeires[s] = [None]
                                            LAITimeSeires[s] =  [None]
                                            ETTimeSeires[s] =  [None]
                                else:
                                    n = 0
                                    for s in FIPSbatch:
                                        if s in validFIPS:
                                            ...
                                            GPPTimeSeires[s].append(GPP_mean_list[n])
                                            RecoTimeSeires[s].append(Reco_mean_list[n])
                                            NEETimeSeires[s].append(NEE_mean_list[n])
                                            LAITimeSeires[s].append(LAI_mean_list[n])
                                            ETTimeSeires[s].append(ET_mean_list[n])
                                            n+=1
                                        else:
                                            yieldTimeSeires[s].append(None)
                                            yieldTimeSeires_std[s].append(None)
                                            GPPTimeSeires[s].append(None)
                                            RecoTimeSeires[s].append(None)
                                            NEETimeSeires[s].append(None)
                                            LAITimeSeires[s].append(None)
                                            ETTimeSeires[s].append(None)
                                            
                            for n,t in enumerate(FIPSbatch):            
                                yield_3I_dic[t] = yieldTimeSeires[t]
                                yield_3I_pom_dic[t] = yieldTimeSeires_std[t]
                                GPP_dic[t] = GPPTimeSeires[t]
                                Reco_dic[t] = RecoTimeSeires[t]
                                NEE_dic[t] = NEETimeSeires[t]
                                LAI_dic[t] = LAITimeSeires[t]
                                ET_dic[t] = ETTimeSeires[t]
                        ...
                        carbon_all_dic[crop] = [GPP_dic,Reco_dic,NEE_dic,LAI_dic,ET_dic]
                        if saveRes:
                    ...
                    if saveRes:
                        util.save_object(carbon_all_dic, filename='%s/carbonCycle.pkl'%(outFolder))
                    ...
                    for crop in ['corn','soybean']:
                        ...
                        if paraMode == 'eachYear':
                            yield_df = pd.read_csv('%s/yield_%s.csv'%(outFolder,crop))
                            yield_df[validFIPS]=yield_df[validFIPS]*coef
                        else:
