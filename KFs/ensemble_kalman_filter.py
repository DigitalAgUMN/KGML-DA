...
class EnsembleKalmanFilter(object):
    ...
    def __init__(self, x, P, dim_z, N, hx, fx, x_paraLoc = None, cellRange=None):
        if dim_z <= 0:
    ...
    def update(self, z, R=None, Q = None, MaskedIndex = None):  # 'Q': the process error, added by Qi 2020/10/9
        """
        ...
        for i in range(N):
            ...
            self.sigmas[i] += dot(self.K, z - sigmas_h[i])
    ...
    def predict(self,dailyIn,updateHidden = False, MaskCells = None):
        """ Predict next position. """

        N = self.N
       
        if updateHidden:
            upstate = self.sigmas.copy()
            if not (MaskCells is None):
                for m in MaskCells:
                    upstate[:,self.cellRange[m][0]:self.cellRange[m][1]] = np.nan
                    # for t in range(self.cellRange[m][0],self.cellRange[m][1]):
                    #     upstate[t] = None
 
            self.sigmas = np.squeeze(self.fx(dailyIn,upstate))   
        else:
            self.sigmas = np.squeeze(self.fx(dailyIn))
...
class EnsembleKalmanFilter_parallel(object):
    """
    This implements the ensemble Kalman filter (EnKF). The EnKF uses
    an ensemble of hundreds to thousands of state vectors that are randomly
    sampled around the estimate, and adds perturbations at each update and
    ...
    def update(self, zList, R=None, Q = None, MaskedIndex = None):  # 'Q': the process error, added by Qi 2020/10/9
        ...
        self.x_postList = []
        self.P_postList = []
        self.zList = deepcopy(zList)
        sampleN = 0
        for sigmas,z,x,P in zip(self.sigmasList,zList,self.xList,self.PList):
            ...
            # transform sigma points into measurement space
            for i in range(N):
                sigmas_h[i,:] = self.hx(sigmas[i,:])
    
            z_mean = np.mean(sigmas_h, axis=0)
    
            P_zz = (outer_product_sum(sigmas_h - z_mean) / (N-1)) + R
            P_xz = outer_product_sum(
                    sigmas - x, sigmas_h - z_mean) / (N - 1)
            
            S = P_zz
            self.SList.append(S)
            SI = self.inv(self.SList[-1])
            K = dot(P_xz, SI)
            ...
            for i in range(N):
                # self.sigmas[i] += dot(self.K, z + e_r[i] - sigmas_h[i])
                sigmas[i,:] += dot(K, z - sigmas_h[i,:])

            self.xList[sampleN] = np.mean(sigmas, axis=0)
            self.PList[sampleN] = P - dot(dot(K, S), K.T)
    
            # save measurement and posterior state
            
            self.x_postList.append(self.xList[-1].copy())
            self.P_postList.append(self.PList[-1].copy())
            sampleN+=1

    def predict(self,dailyIn,updateHidden = False):
        """ Predict next position. """

        N = self.N
       
        if updateHidden:
            self.sigmasList = self.fx(dailyIn,self.sigmasList)
        else:
        ...
        self.xList = [np.mean(np.squeeze(t), axis=0) for t in self.sigmasList]
        self.PList = [outer_product_sum(np.squeeze(s) - x) / (N - 1) for s,x in zip(self.sigmasList,self.xList)]
        sigmas_hList = [np.array([self.hx(np.squeeze(s)[i,:]) for i in range(N)]) for s in self.sigmasList]
        self.P_zList = [outer_product_sum(sigmas_h - np.mean(sigmas_h, axis=0)) / (N - 1) for sigmas_h in sigmas_hList]  # added by Qi 2021/8/13, the P in measuremental space
        
        # save prior
        self.x_priorList = np.copy(self.xList)
        self.P_priorList = np.copy(self.PList)
...
class EnsembleKalmanFilter_UpdatePara(object):
    ...
    def predict(self,dailyIn,updateHidden = False, MaskCells = None, stateIndex = [2], paraIndex = [7,8,9,10,11,12]):
        ...
        if updateHidden:
            ...
            self.para = self.sigmas[:,-len(paraIndex):].copy()
            dailyInUpdate[:,paraIndex] = self.para
            dailyInUpdate = dailyInUpdate[:,np.newaxis,:]
            
            upstate = self.out.copy()
            upstate[:,stateIndex] = self.sigmas[:,:len(stateIndex)].copy()
            if not (MaskCells is None):
                for m in MaskCells:
                    upstate[:,self.cellRange[m][0]:self.cellRange[m][1]] = np.nan
                    # for t in range(self.cellRange[m][0],self.cellRange[m][1]):
            ...
            self.out = np.squeeze(self.fx(dailyInUpdate,upstate))
            state = self.out[:,stateIndex]
            self.sigmas = np.concatenate([state,self.para],axis=1)
            ...
            self.out = np.squeeze(self.fx(dailyInUpdate))
            state = self.out[:,stateIndex]
            self.sigmas = np.concatenate([state,self.para],axis=1)
